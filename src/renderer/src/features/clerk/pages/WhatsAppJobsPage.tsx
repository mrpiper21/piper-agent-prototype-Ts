...
export default function WhatsAppJobsPage() {
  ...
  const { data: localMessages = [], isLoading: isLoadingMessages, error: messagesError, refetch: refetchMessages } = useQuery<LocalMessage[]>({
  ...
  // Convert messages to conversations (not jobs - these are just message threads)
  const whatsappJobs = useMemo(() => {
    const messagesArray = Array.isArray(localMessages) ? localMessages : [];
    console.log('[WhatsAppJobsPage] 🔄 Recalculating whatsappJobs from', messagesArray.length, 'messages');
    ...
    for (const [contact, messages] of messagesByContact) {
      ...
      console.log('[WhatsAppJobsPage] ✅ Including conversation with /print command:', {
        contact,
        messageCount: messages.length,
        printCommandMessages: messages.filter(m => 
      ...
      // Get contact name
      const contactName = sortedMessages
        .map(m => m.contactName)
    ...
    conversations.sort((a, b) => {
      ...
      const latestTimeA = (a.metadata as { latestMessageTimestamp?: number })?.latestMessageTimestamp 
        || (() => {
      ...
      const latestTimeB = (b.metadata as { latestMessageTimestamp?: number })?.latestMessageTimestamp
        || (() => {
      ...
      if (Math.abs(sortResult) < 1000) { // Only log if timestamps are very close
    ...
    console.log('[WhatsAppJobsPage] ✅ Filtered and sorted', conversations.length, 'conversations with /print commands (out of', messagesByContact.size, 'total contacts). Top conversation:', {
      contact: (conversations[0]?.metadata as { whatsappContact?: string })?.whatsappContact,
      latestTime: (conversations[0]?.metadata as { latestMessageTimestamp?: number })?.latestMessageTimestamp,
  ...
  useEffect(() => {
    if (selectedJob && whatsappJobs.length > 0) {
      const selectedContact = (selectedJob.metadata as { whatsappContact?: string })?.whatsappContact;
      if (selectedContact) {
        const updatedJob = whatsappJobs.find(
          (job) => (job.metadata as { whatsappContact?: string })?.whatsappContact === selectedContact
        );
        if (updatedJob) {
          // Update to get latest messages and maintain selection even if list reorders
          const currentMessages = (selectedJob.metadata as { messages?: Array<{ messageId?: string }> })?.messages || [];
          const updatedMessages = (updatedJob.metadata as { messages?: Array<{ messageId?: string }> })?.messages || [];
          // Update if messages changed (by count or IDs)
          ...
          const messagesChanged = currentMessages.length !== updatedMessages.length ||
  ...
  useEffect(() => {
    console.log('[WhatsAppJobsPage] 📊 WhatsApp jobs updated:', {
      ...
      jobs: whatsappJobs.map(j => {
        const messages = (j.metadata as { messages?: Array<{ timestamp?: number }> })?.messages || [];
        const latestTimestamp = (j.metadata as { latestMessageTimestamp?: number })?.latestMessageTimestamp
  ...
  // Optimized handler for real-time message updates
  const handleMessageUpdate = useCallback(async () => {
    ...
    // Update trigger to force query key change
    ...
    try {
      ...
      console.log('[WhatsAppJobsPage] ✅ Messages refetched successfully');
    } catch (error) {
      console.error('[WhatsAppJobsPage] Error refetching messages:', error);
    }
    
    // Also invalidate and refetch via query client as backup
  ...
  // Listen for real-time WhatsApp messages - optimized
  useEffect(() => {
    ...
    const unsubscribeMessage = electronAPI.whatsapp.onMessage((messageData: { contact?: string; message?: { messageId?: string; body?: string; from?: string }; messageId?: string; body?: string; from?: string; contactName?: string }) => {
      ...
      // Immediately trigger UI update when any message arrives (client or agent)
      // Use a small delay to ensure message is stored, then refetch aggressively
      setTimeout(() => {
        console.log('[WhatsAppJobsPage] ⏰ Triggering immediate message update');
        handleMessageUpdate();
      }, 100);
      
      // Also trigger again after a slightly longer delay as backup
      setTimeout(() => {
        console.log('[WhatsAppJobsPage] ⏰ Backup refetch');
        handleMessageUpdate();
      }, 500);
    });
    ...
    const unsubscribeHistory = electronAPI.whatsapp.onHistoryLoaded((data: { count: number }) => {
      ...
      // Refresh when history is loaded
      handleMessageUpdate();
    });
      ...
      >
        ...
        >
          {isLoadingMessages ? (
            ...
            >
              <p>Error loading conversations: {messagesError instanceof Error ? messagesError.message : 'Unknown error'}</p>
            </div>
          ) : (
            <WhatsAppConversationList
