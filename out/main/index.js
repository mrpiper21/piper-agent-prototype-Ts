...
class DatabaseService {
  ...
  init() {
    try {
      logger$1.info("Database initialized (in-memory)");
    } catch (error) {
      logger$1.error("Database initialization error", error);
      throw error;
    }
  }
  createTables() {
  ...
  getUserById(id) {
    return this.data.users.find((u2) => u2.id === id);
  }
  createUser(data) {
    const user = {
    ...
    logger$1.info("User created", { id: user.id, email: user.email });
    return user;
  }
  updateUser(id, data) {
  ...
  deleteUser(id) {
    this.data.users = this.data.users.filter((u2) => u2.id !== id);
    logger$1.info("User deleted", { id });
  }
  // Auth
  createAuthSession(token, userId) {
...
var cwd = null;
var platform$3 = process.env.GRACEFUL_FS_PLATFORM || process.platform;
...
async function checkPaths(src2, dest, funcName, opts) {
  ...
  if (destStat) {
    if (areIdentical$2(srcStat, destStat)) {
      const srcBaseName = path$b.basename(src2);
...
async function checkParentPaths(src2, srcStat, dest, funcName) {
  const srcParent = path$b.resolve(path$b.dirname(src2));
...
function isSrcSubdir(src2, dest) {
  const srcArr = path$b.resolve(src2).split(path$b.sep).filter((i) => i);
...
async function asyncIteratorConcurrentProcess$1(iterator2, fn) {
  const promises = [];
...
const stat$3 = stat$4;
const { asyncIteratorConcurrentProcess } = async$3;
...
async function onDir$1(srcStat, destStat, src2, dest, opts) {
  ...
  await asyncIteratorConcurrentProcess(await fs$d.opendir(src2), async (item) => {
    const srcItem = path$a.join(src2, item.name);
...
class Logger2 {
  ...
  ensureLogDir() {
    if (this.logToFile) {
      fs$1.ensureDirSync(this.logDir);
    }
  }
  ...
  async writeToFile(message) {
    ...
    const logFile = path$d.join(this.logDir, "agent.log");
    try {
      await fs$1.appendFile(logFile, message + "\n");
  ...
  async rotateLogIfNeeded(logFile) {
    try {
      const stats = await fs$1.stat(logFile);
      if (stats.size > this.maxLogSize) {
        for (let i = this.maxLogFiles - 1; i >= 1; i--) {
          const oldFile = `${logFile}.${i}`;
          const newFile = `${logFile}.${i + 1}`;
          if (await fs$1.pathExists(oldFile)) {
            await fs$1.move(oldFile, newFile);
          }
        }
        await fs$1.move(logFile, `${logFile}.1`);
      }
    } catch (error) {
...
class PlatformUtils {
  ...
  detectPlatform() {
    ...
    return {
      ...
      version,
      arch,
      isSupported: SUPPORTED_PLATFORMS.includes(platform2)
    };
  }
  getPlatformInfo() {
  ...
  getApplicationDataDirectory() {
    if (this.isWindows()) {
      return path$d.join(os.homedir(), "AppData", "Local", "PrintMyFile");
    } else if (this.isMacOS()) {
      return path$d.join(os.homedir(), "Library", "Application Support", "PrintMyFile");
    } else {
      return path$d.join(os.homedir(), ".config", "printmyfile");
    }
  }
  getLogsDirectory() {
    return path$d.join(this.getApplicationDataDirectory(), "logs");
  }
  getConfigDirectory() {
    return path$d.join(this.getApplicationDataDirectory(), "config");
  }
  getDownloadsDirectory() {
    return path$d.join(this.getApplicationDataDirectory(), "downloads");
  }
  getSystemInfo() {
    return {
  ...
  normalizePath(filePath) {
    return path$d.normalize(filePath);
  }
  joinPath(...paths) {
    return path$d.join(...paths);
  }
  resolvePath(...paths) {
    return path$d.resolve(...paths);
  }
  getFileExtension(filePath) {
    return path$d.extname(filePath).toLowerCase();
  }
  getFileName(filePath) {
    return path$d.basename(filePath);
  }
  getDirectoryName(filePath) {
    return path$d.dirname(filePath);
  }
  ...
  getServiceInstallCommand(serviceName, executablePath) {
    if (this.isWindows()) {
  ...
  getServiceRemoveCommand(serviceName) {
    if (this.isWindows()) {
...
const platform$2 = PlatformUtils.getInstance();
...
class WindowsDriver {
   ...
   */
  parseWindowsPrinterList(output) {
   ...
   */
  parseWindowsCapabilities(output) {
    const capabilities = {
      color: false,
      duplex: false,
  ...
  async executeCommand(command) {
    try {
      const { stdout, stderr } = await execAsync$4(command, {
        timeout: 1e4,
        encoding: "utf8",
        shell: "cmd.exe"
      });
   ...
   */
  async getPrinterQueue(printerName) {
    try {
  ...
  parseWindowsPrintQueue(output) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        const parts = line?.split(",") || [];
        if (parts.length >= 5) {
          const [, jobId, document2, status, size] = parts;
          jobs.push({
            jobId: jobId?.trim() || "",
            document: document2?.trim() || "",
            status: status?.trim() || "",
            size: size?.trim() || ""
          });
        }
...
class MacOSDriver {
  ...
  async getDefaultPrinter() {
    try {
      ...
      if (error.stderr?.includes("No destinations added") || error.stderr?.includes("no system default destination")) {
        logger.info("No default printer set on macOS system");
        return null;
      }
   ...
   */
  parseMacOSStatus(status, enabled) {
  ...
  async executeCommand(command) {
    try {
      const { stdout, stderr } = await execAsync$3(command, {
        timeout: 1e4,
        encoding: "utf8",
        shell: "/bin/bash"
      });
  ...
  parseMacOSPrintQueue(output) {
    ...
    for (let i = 2; i < lines.length; i++) {
      ...
      try {
        ...
        if (parts.length >= 4) {
          const [, , jobId, document2, ...sizeParts] = parts;
          jobs.push({
            jobId: jobId || "",
            document: document2 || "",
            status: "queued",
   ...
   */
  async enablePrinter(printerName) {
    try {
   ...
   */
  async disablePrinter(printerName) {
    try {
...
class LinuxDriver {
   ...
   */
  parseLinuxStatus(status, enabled) {
    if (!enabled) return "offline";
    const lowerStatus = status.toLowerCase();
    if (lowerStatus === "idle") return "online";
    if (lowerStatus === "printing") return "busy";
...
class DiscoveryService {
  ...
  async discoverPrinters() {
    try {
      const platformName = platform$2.getPlatformInfo().name;
   ...
   */
  validateAndEnhancePrinter(printer) {
  ...
  async getDiscoveryStats() {
    ...
    try {
      ...
      return {
        ...
        lastDiscovery: /* @__PURE__ */ new Date(),
...
class OSCommands {
  constructor() {
    this.platformName = platform$2.getPlatformInfo().name;
  }
  /**
   * Get printer status
   */
   ...
   */
  buildPrintCommand(printerName, filePath, options) {
    const args = [];
   ...
   */
  parsePrinterStatus(output, _platform) {
    const lowerOutput = output.toLowerCase();
    switch (_platform) {
      case "win32":
   ...
   */
  parsePrinterCapabilities(output, _platform) {
    const capabilities = {
      color: false,
      duplex: false,
   ...
   */
  async isPrinterCommandAvailable() {
    try {
      let command;
      switch (this.platformName) {
        case "win32":
   ...
   */
  parseSystemPrinterInfo(output, _platform) {
    const lines = output.split("\n").filter((line) => line.trim());
    return {
      totalPrinters: lines.length,
      defaultPrinter: null,
      // Would need additional parsing logic
...
const __filename$1 = require$$0$2.fileURLToPath(require("url").pathToFileURL(__filename).href);
const __dirname$1 = path$d.dirname(__filename$1);
...
`);
    const pdfPath = path$d.join(__dirname$1, "src/testfiles/Deed Of Assignment (Mr Ayi Mensah).pdf");
...
class PrinterManager {
  constructor() {
    this.discoveredPrinters = /* @__PURE__ */ new Map();
   ...
   */
  startPrinterMonitoring(interval = 3e4) {
   ...
   */
  validatePrinterConfig(printerName) {
    const errors = [];
    if (!printerName) {
   ...
   */
  async getAvailablePaperSizes(printerName) {
    try {
   ...
   */
  async supportsColor(printerName) {
    try {
      const capabilities = await this.getPrinterCapabilities(printerName);
   ...
   */
  async supportsDuplex(printerName) {
    try {
      const capabilities = await this.getPrinterCapabilities(printerName);
  ...
  */
  async testPrint() {
    try {
      ...
      const printerName = printers[0].printerName;
      const pdfPath = path$d.join(process.cwd(), "src/testfiles/Deed Of Assignment (Mr Ayi Mensah).pdf");
      logger.info(`
🧪 Test Print`);
...
`);
   ...
   */
  async discoverAndRegisterPrinters() {
    try {
...
📋 Found ${jobs.length} job(s) to process`);
          ...
          this.agentState.lastPoll = /* @__PURE__ */ new Date();
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
...
class AgentService {
  ...
  async start() {
    try {
      if (this.isRunning) {
        logger$1.warn("Agent is already running");
        return;
      }
      logger$1.info("Starting agent...");
      this.agentInstance = new Agent();
      this.agentInstance.initialize();
      await this.agentInstance.start();
      this.isRunning = true;
      logger$1.info("Agent started successfully");
    } catch (error) {
      logger$1.error("Failed to start agent:", error);
      this.isRunning = false;
      throw error;
    }
  }
  ...
  async stop() {
    try {
      if (!this.isRunning || !this.agentInstance) {
        logger$1.warn("Agent is not running");
        return;
      }
      logger$1.info("Stopping agent...");
      await this.agentInstance.shutdown();
      this.isRunning = false;
      this.agentInstance = null;
      logger$1.info("Agent stopped successfully");
    } catch (error) {
      logger$1.error("Failed to stop agent:", error);
      throw error;
    }
  }
  /**
  ...
  getStatus() {
    ...
    const state2 = this.agentInstance.getStatus();
    return {
      status: state2.status,
      printerCount: state2.printers.length,
      jobsProcessed: state2.jobsProcessed,
      lastPoll: state2.lastPoll,
      uptime: state2.uptime,
   ...
   * Test print a file
   */
  async testPrint(printerName, filePath) {
    if (!this.agentInstance) {
  ...
  async printFile(printerName, filePath, options) {
    ...
    logger$1.info(`Printing ${filePath} to ${printerName}`);
...
class ApiService {
  ...
  getToken() {
    ...
    try {
      const tokenPath = path$d.join(process.cwd(), ".auth-token");
      if (fs$k.existsSync(tokenPath)) {
        return fs$k.readFileSync(tokenPath, "utf8").trim();
      }
    } catch (error) {
    }
    return null;
  ...
  setToken(token) {
    if (typeof localStorage !== "undefined") {
      ...
      try {
        const tokenPath = path$d.join(process.cwd(), ".auth-token");
        fs$k.writeFileSync(tokenPath, token, "utf8");
      } catch (error) {
      }
    }
  }
  ...
  async submitJobToPrinter(jobId, agentId) {
...
function setupIpcHandlers() {
  electron.ipcMain.handle("auth:login", async (_, credentials) => {
    try {
      const response = await apiService.login(credentials.email, credentials.password);
      logger$1.info("User logged in", { email: credentials.email });
  ...
  electron.ipcMain.handle("auth:logout", async (_, token) => {
    try {
      ...
      logger$1.error("Logout error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("auth:refresh", async (_, token) => {
    try {
      const response = await apiService.refreshToken();
      logger$1.info("Token refreshed");
      return response.token;
    } catch (error) {
      logger$1.error("Token refresh error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("users:getAll", async () => {
    try {
      return dbService.getAllUsers();
    } catch (error) {
      logger$1.error("Get users error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("users:getById", async (_, id) => {
    try {
      ...
      logger$1.error("Get user error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("users:create", async (_, data) => {
    try {
      ...
      logger$1.error("Create user error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("users:update", async (_, id, data) => {
    try {
      ...
      logger$1.error("Update user error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("users:delete", async (_, id) => {
    try {
      ...
      logger$1.error("Delete user error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("files:save", async (_, filePath, content) => {
    try {
      const fs2 = require("fs").promises;
      await fs2.writeFile(filePath, content, "utf-8");
      logger$1.info("File saved", { path: filePath });
    } catch (error) {
      logger$1.error("File save error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("files:read", async (_, filePath) => {
    try {
      const fs2 = require("fs").promises;
      const content = await fs2.readFile(filePath, "utf-8");
      return content;
    } catch (error) {
      logger$1.error("File read error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:start", async () => {
    try {
      await agentService.start();
      logger$1.info("Agent started via IPC");
      return { success: true };
    } catch (error) {
      logger$1.error("Agent start error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:stop", async () => {
    try {
      await agentService.stop();
      logger$1.info("Agent stopped via IPC");
      return { success: true };
    } catch (error) {
      logger$1.error("Agent stop error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:getStatus", async () => {
    try {
      return agentService.getStatus();
    } catch (error) {
      logger$1.error("Agent getStatus error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:getPrinters", async () => {
    try {
      return agentService.getPrinters();
    } catch (error) {
      logger$1.error("Agent getPrinters error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:discoverPrinters", async () => {
    try {
      return await agentService.discoverPrinters();
    } catch (error) {
      logger$1.error("Agent discoverPrinters error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:printFile", async (_, printerName, filePath, options) => {
    try {
      await agentService.printFile(printerName, filePath, options);
      logger$1.info("Print job sent via IPC", { printerName, filePath });
    } catch (error) {
      logger$1.error("Agent printFile error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:testPrint", async (_, printerName, filePath) => {
    try {
      await agentService.testPrint(printerName, filePath);
      logger$1.info("Test print sent via IPC", { printerName, filePath });
    } catch (error) {
      logger$1.error("Agent testPrint error", error);
      throw error;
    }
  });
  electron.ipcMain.handle("agent:isRunning", async () => {
    try {
      return agentService.isAgentRunning();
    } catch (error) {
      logger$1.error("Agent isRunning error", error);
      throw error;
    }
  });
  logger$1.info("IPC handlers registered");
}
if (process.env.NODE_ENV === "development") {
  try {
    require("electron-reload")(__dirname, {
      electron: path$d.join(__dirname, "../../node_modules/.bin/electron"),
...
electron.app.on("before-quit", () => {
  logger$1.info("Application shutting down...");
  dbService.close();
});
